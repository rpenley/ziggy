...
/// `{template_dir}/{theme}/partials/{name}.mustache` and inlining its content.
...
pub fn expandPartials(
	...
	while (pos < template.len) {
		...
		const partial_path = try std.fmt.bufPrint(&partial_buf, "{s}/{s}/partials/{s}.mustache", .{
			config.template_dir, config.theme, name,
		});
...
pub fn loadTemplate(allocator: std.mem.Allocator, config: *const Config, stem: []const u8) ![]const u8 {
	...
	const raw = blk: {
		...
		if (std.fs.cwd().readFileAlloc(allocator, fallback, max_content_bytes)) |content| {
			break :blk content;
		} else |_| {}
		...
		if (!std.mem.eql(u8, config.theme, "default")) {
			...
			break :blk try std.fs.cwd().readFileAlloc(allocator, last_resort, max_content_bytes);
		}
...
/// Loads `{template_dir}/{theme}/partials/{name}.mustache`.
...
pub fn loadPartial(allocator: std.mem.Allocator, config: *const Config, name: []const u8) ![]const u8 {
	var path_buf: [1024]u8 = undefined;
	const path = try std.fmt.bufPrint(&path_buf, "{s}/{s}/partials/{s}.mustache", .{
		config.template_dir, config.theme, name,
	});
